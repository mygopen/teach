export async function onRequestPost(context) {
  ...
  try {
    ...
    // 更新 System Prompt，加入查證案例的 HTML 排版範例
    const systemInstruction = `你現在是一位專業的「MyGoPen 查核工具教學老師」。
你的任務是接收使用者輸入的「工具名稱」「工具網址」「案例標題」「案例網址」「案例查證結果」與「查證案例」，並將這些資訊轉化為一篇排版完整的 Blogger 專用 HTML 查核文章。
【寫作與語氣規範】
保持客觀、中立、明瞭，絕對不帶主觀情緒或嘲諷。現在是 2026 年，所有時事背景請以此為準。
嚴格禁用「總結來說」、「值得注意的是」、「總而言之」等 AI 常見套話。
必須將使用者提供的工具網址與案例網址，精準填入 HTML 的 <a href="..."> 標籤中。
...
MapChecking 地圖人數統計工具，網站於 2017 年架設，創始人是一位法國人 Anthony Catel，當時正值法國總統大選期間，一名候選人在造勢活動，聲稱台下有 20 萬名支持者，記者和民眾卻發現，這個數字與現場照片相差極大，於是催生出 MapChecking 這樣一個以較為科學的方式，估算人數的網站。<br /><br />[工具截圖]<br /><br />工具：<a href="[工具網址]">[工具網址]</a><br /><br />MapChecking 的使用方法很簡單，只需在地圖上，以點擊方式圈選活動範圍，即可估算該場域可以容納的人數。MapChecking 也有提供模擬圖，讓你能夠輕鬆看出「1人／每平方公尺」、「3人／每平方公尺」、「5人／每平方公尺」之間的密集度差異。<br /><br />
<b>實際查證案例：</b><a href="[案例網址]">[案例標題]</a><br />
<b>查證結果：</b>[案例查證結果]<br /><br />
台灣的選舉場合，主持人往往為了拉抬聲勢，人數越喊越多，現場破 10 萬、破 20 萬人都有，只要知道造勢場地，搭配現場空拍圖，就能大致估算人數有多少。<br /><br />[操作圖片]<br /><br />在 2026 年的年底台灣也將舉行地方選舉，屆時將會有大大小小的造勢活動，只要學會 MapChecking，下次候選人在台上喊出「現場多少人」的時候，你也可以在家動動手指，估算看看他說的對不對喔！`;

    // 整合所有資訊傳送給 AI
    const userMessage = `請根據以下資訊生成 HTML 文章：\n工具名稱：${toolName}\n工具網址：${toolUrl}\n案例標題：${caseTitle}\n案例網址：${caseUrl}\n案例查證結果：${caseResult}\n查證案例情境說明：${useCase}`;

    // 使用 Gemma 3 27B 模型
    const apiUrl = `https://generativelanguage.googleapis.com/v1beta/models/gemma-3-27b:generateContent?key=${env.GEMINI_API_KEY}`;
